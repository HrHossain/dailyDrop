...
export default function AdminLayout() {
    ...
    <div className="flex flex-col h-full lg:flex-row gap-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 animate-fade-in">
        {/* Admin Sidebar */}
        <aside className="w-full lg:w-64 shrink-0 h-fit bg-white rounded-card p-4 border border-mist-200">
            <div className="pb-4 mb-4 border-b border-mist-200">
                <h2 className="text-lg font-semibold text-forest-700 flex items-center gap-2 px-2">
                    <ShieldIcon className="size-5 text-forest-700" /> Admin Panel
                </h2>
            </div>
                ...
                {AdminLinkData.map((link) => (
                    <NavLink
                        ...
                        className={({ isActive }) => `flex items-center gap-3 p-2.5 rounded-md text-sm transition-colors ${isActive
                            ...
                            : "text-charcoal-400 hover:bg-mist-100 hover:text-charcoal"
                            }`}
                    >
