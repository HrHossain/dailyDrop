        ...
        <div className="flex gap-2 mb-6 overflow-x-auto pb-2">
          {tabs.map(tab => (
            <button 
              ...
              className={`px-4 py-2 mt-6 text-sm font-medium rounded-xl whitespace-nowrap transition-colors ${
                activeTab === tab ? "bg-leaf text-white" : "bg-white text-charcoal hover:text-charcoal-500"
              }`}
            >
              {tab}
            </button>
            ...
            {orders.map((order: Order) => (
              ...
              >
                ...
                <div className="flex justify-between items-center">
                  <div>
                    <p className="text-sm font-medium text-leaf">Order #{order.id.slice(-8).toUpperCase()}</p>
                ...
                <div className="flex items-center gap-2 my-3">
                  ...
                  {order.items.length > 4 && (
                    <div className="size-12 sm:size-16 rounded-lg bg-mist flex items-center justify-center text-xs font-semibold text-charcoal">
                      +{order.items.length - 4}
                    </div>
                  )}
                </div>
                ...
                <div className="flex justify-between items-center pt-3 text-sm">
                  <span className="text-charcoal-600">{order.items.length} items</span>
                  <span className="text-charcoal text-[18px] font-bold">{order.total.toFixed(2)} {currency}</span>
                </div>
              </Link>
            ))}
          </div>
